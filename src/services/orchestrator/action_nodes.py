...
async def _bank_search(context_text: str, category: str | None, exclude: list[str]):
    """Search question bank for relevant questions; returns [] on any failure."""
    try:
        from src.core.database import AsyncSessionLocal
        from src.services.data.question_bank_service import search_for_interview, count_questions
        async with AsyncSessionLocal() as db:
            total = await count_questions(db)
            if total == 0:
                return []
            return await search_for_interview(db, context_text, category=category, limit=3, exclude_questions=exclude)
    except Exception as e:
        logger.debug(f"Bank search skipped: {e}")
        return []
...
class ActionNodeMixin:
    ...
    async def question_node(self, state: InterviewState) -> InterviewState:
        ...
        # Pull relevant questions from the bank (best-effort, non-blocking)
        bank_context = ""
        try:
            job_desc = state.get("job_description", "") or ""
            resume_ctx = build_resume_context(state)
            search_text = f"{job_desc}\n{resume_ctx}\nTopics covered: {', '.join(topics_covered)}"
            bank_results = await _bank_search(search_text, category=None, exclude=questions_asked)
            if bank_results:
                bank_lines = "\n".join(
                    f"- [{r.category} / {r.level}] {r.topic}: {r.question}"
                    for r in bank_results
                )
                bank_context = f"\n\nQuestion Bank Suggestions (use verbatim or adapt naturally):\n{bank_lines}"
        except Exception:
            pass
        ...
        try:
            response = await self.llm_helper.call_llm_with_instructor(
                system_prompt=get_system_prompt() + " Generate questions that are genuine, relevant, and flow naturally from the conversation. Acknowledge when it makes sense, but don't force it. Show you're listening through natural conversation.",
                user_prompt=prompt,
    ...
    async def check_sandbox_code_changes(self, state: InterviewState) -> dict:
        ...
        if current_time - last_poll < get_sandbox_poll_interval():
            return updates
        ...
        is_stuck = time_since_activity > get_sandbox_stuck_threshold() and current_code != initial_code
