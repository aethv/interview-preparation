...
class SandboxService:
    ...
    def _get_language_image(self, language: Language) -> str:
        ...
        images = {
            ...
            Language.JAVA: "eclipse-temurin:21-jdk-slim",
        }
    ...
    def _execute_in_container(
        ...
        try:
            if language == Language.PYTHON:
                ...
                # compile then execute in one shell call
                command = ["sh", "-c", "cd /workspace && javac Solution.java && java Solution"]
            else:
                command = ["python", "/workspace/main.py"]
    ...
    async def _execute_fallback(
        ...
        try:
            if language == Language.PYTHON:
                ...
                return ExecutionResult(
                    error="Java requires Docker sandbox (not available in fallback mode)",
                    exit_code=1,
                )
            else:
                return ExecutionResult(
